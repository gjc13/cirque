...
class TestGrpcVirtualHome(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        for home_id in home_ids:
            ...
            cls.stub.StopCirqueHome(service_pb2.StopCirqueHomeRequest(
                home_id=home_id))
    ...
    def test_003_connect_to_thread_network(self):
        ...
        devices = self.stub.ListCirqueHomeDevices(
            service_pb2.ListCirqueHomeDevicesRequest(
                home_id=home_id))
        devices = [d for d in devices.devices
    ...
    def test_004_scan_available_wifi_network(self):
        ...
        ssid = [device.device_description.ssid
                for device in devices.devices
        ...
        for device_id in device_ids:
            ...
            ssid_from_dev = re.search(
                r'\s+ESSID:"(.+)"', ret.output).group(1)
            self.logger.info("\nssid: {} scanned by\ndevice: {}".format(
    ...
    def test_005_connect_to_desired_wifi_network(self):
        ...
        for device_id in device_ids:
            ...
            write_psk_to_wpa_supplicant_config(
                self.logger, self.stub, home_id, device_id, ssid, psk)

            kill_existing_wpa_supplicant(
                self.logger, self.stub, home_id, device_id)
    ...
    def test_006_device_connectivity(self):
        ...
        self.assertNotEqual(loss, '100',
                            "unable to ping device: {}, no wifi connectivity!!"
                            .format(device_addrs[1][1]))
